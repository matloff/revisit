...
zav = zav[zav$year < 2008,] # 2000-2007 (since first year in zav.txt is 2000)

##### traditional employment rate and immigrant share model #####
# employment rate for natives = employed natives divided by total native population
zav$emprate_native   <- zav$emp_native / zav$pop_native * 100
# immigrant stem workers with advanced degrees from U.S. universities - share of total workers employed
zav$immshare_emp_stem_e_grad   <- zav$emp_edus_stem_grad / zav$emp_total * 100
# immigrant stem workers with advanced degrees from non-U.S. universities - share of total workers employed
...
# Years in sample (in this case, 2000-2007), converted to factors for use as an instrumental variable
zav$fyear  <- as.factor(zav$year)
# 50 states plus Washington D.C., converted to factors for use as an instrumental variable
...
# do regression with two independent variables, two instrumental variables, and weights to treat years equally
